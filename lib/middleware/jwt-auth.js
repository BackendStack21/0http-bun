...
function createJWTAuth(options = {}) {
  ...
  return async function jwtAuthMiddleware(req, next) {
    // Skip authentication for excluded paths (exact or boundary match on canonical path)
    ...
    try {
      ...
      if (optional && (!hasApiKeyMode || !req.headers.get(apiKeyHeader))) {
        req.ctx = req.ctx || {}
        // Do not leak jose/library internals via request context
        req.ctx.authError = 'Invalid or expired token'
...
function extractToken(req, options = {}) {
  ...
  // Try query parameter (prefer already-parsed req.query)
  if (tokenQuery) {
    const fromQuery = req.query && req.query[tokenQuery]
    ...
    try {
      ...
      // Malformed URL — ignore query extraction
    }
  }
