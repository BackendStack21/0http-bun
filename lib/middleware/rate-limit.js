...
class MemoryStore {
  ...
  increment(key, windowMs) {
    ...
    // Amortize cleanup: full scan every N increments instead of every request
    this._ops++
    if (this._ops >= this.cleanupEvery) {
      this._ops = 0
      this.cleanup(now)
    }
    ...
    // Fail closed for *new* keys when the store is full so eviction cannot
    // reset a victim's counter. Existing keys still increment.
    if (current === 0 && !this.store.has(storeKey)) {
...
function createRateLimit(options = {}) {
  ...
  const resolvedStore = store || new MemoryStore({maxKeys})
  ...
  return async function rateLimitMiddleware(req, next) {
    ...
    if (isExcludedPath(getRequestPath(req), excludePaths)) {
      return next()
    }
...
function createSlidingWindowRateLimit(options = {}) {
  ...
  return async function slidingWindowRateLimitMiddleware(req, next) {
    ...
    // Fail closed for new keys when the store is full (do not evict victims)
    if (!hadKey && requests.size >= maxKeys) {
      const response = await handler(
        req,
        max,
        max,
