...
/**
 * String-aware JSON nesting scan. Returns true if nesting exceeds the limit.
 * Short-circuits as soon as the limit is exceeded.
 * @param {string} text
 * @param {number} maxNesting
 * @returns {boolean}
 */
function isJsonNestingTooDeep(text, maxNesting = MAX_JSON_NESTING) {
  let nestingLevel = 0
  let inString = false
  let escape = false
  for (let i = 0; i < text.length; i++) {
    ...
    if (ch === '{' || ch === '[') {
      nestingLevel++
      if (nestingLevel > maxNesting) return true
    } else if (ch === '}' || ch === ']') {
      nestingLevel--
    }
  }
...
function hasBody(req) {
  return BODY_METHODS.has(req.method)
}
...
function createJSONParser(options = {}) {
  ...
  return async function jsonParserMiddleware(req, next) {
    ...
    // Additional protection against excessively deep nesting (short-circuits)
    if (text.length > 0 && isJsonNestingTooDeep(text)) {
...
function createBodyParser(options = {}) {
  ...
  const loweredJsonTypes = jsonTypes.map((t) => t.toLowerCase())
  ...
  // Cache per-type JSON parsers so custom jsonTypes do not allocate every request
  const customJsonParserCache = new Map()
  function getCustomTypeJsonParser(type) {
    let parser = customJsonParserCache.get(type)
    if (!parser) {
      ...
      customJsonParserCache.set(type, parser)
    }
    return parser
  }
  ...
  return async function bodyParserMiddleware(req, next) {
    ...
    try {
      ...
      if (jsonParser && isJsonType(contentType, loweredJsonTypes)) {
        const contentLength = req.headers.get('content-length')
        ...
        if (isJsonType(contentType, loweredJsonTypes)) {
          // Use a cached JSON parser that matches the detected custom type
          const matchedType =
            jsonTypes.find((t) =>
              contentType.toLowerCase().includes(t.toLowerCase()),
            ) || 'application/json'
          result = await getCustomTypeJsonParser(matchedType)(req, next)
        } else {
